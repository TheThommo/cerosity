...
// pointing back at Cerosity (/api/vapi/chat/completions), so every spoken
// reply is generated by buildFloPrompt() + Gemini — the single FLO brain.
// No system prompt lives here: putting one here would be dual intelligence.
...
export function buildFloVapiAssistantConfig(baseUrl: string = publicBaseUrl()) {
  ...
  return {
    name: "FLO - Cerosity",
    // Custom-LLM: VAPI POSTs OpenAI-format requests to `${url}/chat/completions`.
    // Cerosity is the brain. No systemMessage / no hosted coaching model here.
    model: {
      provider: "custom-llm",
      url: `${base}/api/vapi`,
      model: "cerosity-flo",
    ...
    server: { url: `${base}/api/webhooks/vapi-events` },
    silenceTimeoutSeconds: 10,
    maxDurationSeconds: 1800,
    metadata: { platform: "cerosity", brain: "cerosity-custom-llm" },
  };
}

// Idempotently force the live VAPI assistant onto the custom-LLM (Cerosity brain)
// config. Runs on server startup so any drift back to a hosted LLM / dashboard
// prompt is overwritten on deploy. Never throws — voice must not block boot.
export async function reconcileFloVapiAssistant(): Promise<{ ok: boolean; detail?: string }> {
  if (!VAPI_API_KEY) {
    console.log("[VAPI] Skipping assistant reconcile — VAPI_API_KEY not set");
    return { ok: false, detail: "no_api_key" };
  }
  try {
    const config = buildFloVapiAssistantConfig();
    await updateVapiAssistant(FLO_VAPI_ASSISTANT_ID, config);
    console.log(`[VAPI] FLO assistant reconciled to custom-llm brain (${FLO_VAPI_ASSISTANT_ID})`);
    return { ok: true };
  } catch (err: any) {
    console.error("[VAPI] Assistant reconcile failed:", err?.message || err);
    return { ok: false, detail: err?.message || "reconcile_failed" };
  }
}
